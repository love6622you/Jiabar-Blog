import { z } from "zod";
import { FILE_ACCEPTED_IMAGE_TYPES, MAX_FILE_SIZE, MAX_INPUT_LIMIT } from "@/constant/Input";
...
export const PostFormSchema = z.object({
  ...
  title: z.string({ required_error: "輸入文章標題" }).nonempty(TYPE_ERROR.required),
  tags: z.array(z.string()).nonempty(TYPE_ERROR.required),
  content: z.string({ required_error: "輸入文章內容" }).nonempty(TYPE_ERROR.required)
});
